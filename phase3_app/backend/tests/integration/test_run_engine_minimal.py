...
def test_engine_full_flow_accepts_fake_model_client():
    company_products = [
        CompanyProduct(
            code="P1",
            name="海天金标生抽",
    ...
    result = MatchRunEngine(model_client=FakeModelClient()).run_full(
        customer_items,
        company_products,
        max_rounds=1,
    )
    ...
    assert result.row_results["row-1"].selected_candidate is not None
